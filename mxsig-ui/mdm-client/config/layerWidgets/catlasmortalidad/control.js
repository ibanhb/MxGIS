// JavaScript Document
window.layerWidgets.catlasmortalidad = {
	widgetLoaded:false,
	widgetActive:false,
	init:function(opc){
		var obj = this;
		...
		obj.path = 'config/layerWidgets/'+obj.layer.idLayer+'/';
		var path = obj.path;
	...
	events:function(opc){ //eventos del MDM
		...
		switch(obj.event.event){
			case 'identify':
				$('#'+'mdmwidgetlayer'+id)[obj.layer.idLayer]('onIdentify',obj.event.data);
			break;
	...
	initWidget:function(){
			...
			$('#'+'mdmwidgetlayer'+id)[obj.layer.idLayer]({
						map:map,
						config:config,
						path:obj.path,
						onActive:function(){
...
//@ sourceURL=config/layerWidgets/widget/catlasmortalidad/control.js