$.widget("custom.eod", {
				...
				formatter: function () {
					...
					$.each(this.points, function (i, point) {
						s.push('<span style="color:' + point.series.color + ';font-weight:bold;">' +
							'Hora: ' + Highcharts.dateFormat('%H:%M', this.x) + ' <br/>' +
	...
	updatePlayControls:function(){
		...
		var cadena = '<table class="eod-graph-table-control-time eod-animated"><tr>';
		for(var x in hours){
			var item = hours[x];
			var t = parseInt(x);
			var hour = ((t) < 10)?'0'+(t):t;
		...
		$('.eod-graph-table-control-time td').each(function(){
			$(this).click(function(){
