// JavaScript Document
var cfepade = {baseMapBeforeStart:null};
window.layerWidgets.cfepade = {
	widgetLoaded:false,
	widgetActive:false,
	init:function(opc){
		var obj = this;
	...
	events:function(opc){ //eventos del MDM
		...
		switch(obj.event.event){
			case 'identify':
				$('#'+'mdmwidgetlayer'+id).cfepade('onIdentify',obj.event.data);
			break;
	...
	initWidget:function(){
			...
			$('#'+'mdmwidgetlayer'+id).cfepade({
						map:map,
						config:config,
						onActive:function(){
							
							cfepade.baseMapBeforeStart = $("#mdm6Layers").layerManager('getCurrentBaseMap');
						...
						onClose:function(){
							...
							$("#mdm6Layers").layerManager('setBaseMap',cfepade.baseMapBeforeStart);
...
//@ sourceURL=config/layerWidgets/cfepade/control.js