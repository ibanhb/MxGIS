define(['tutorialDenue'],function(tutorialDenue) {
	...
	getCenter:function(){
		...
		'<div class="Tutorial-center">'+
		    ...
		    '<div class="fonts'+clase+'">'+
				'<div class="title">Aumenta</div>'+
				'<div class="options">Disminuye</div>'+
		    '</div>'+
	...
	getBottomPanel:function(){
	    var chain = //'<div class="Tutorial-bottomPanel">'+
				...
				'<div style="right:0px" class="vSeparator line-vertical"></div>'+
				'<div class="fonts">'+
				    '<div class="title">Activa otros mapas base</div>'+
				'</div>'+
			    '</div>'+
			'</div>';
	    return chain;
	...
	getControlZoom:function(){
	    var chain = '<div class="Tutorial-controlZoom">'+
			    ...
			    '<div class="fonts">'+
				    '<div style="position:absolute;top: 0px;right:-36px" class="title">Aumenta</div>'+
				    '<div style="position:absolute;top: 51px;right: -34px;" class="options">Disminuye</div>'+
			    '</div>'+
			'</div>';
	    return chain;
	},
	...
	getRightTools:function(){
	   var chain = '<div class="Tutorial-rightTools">'+
						...
						'<div class="fonts">'+
							'<div style="position: absolute; right:257px; top:102px" class="title">descarga</div>'+
	...
	show:function(){
        ...
        $(".Tutorial-search").animate({top: "157px"}, 700);
        $(".Tutorial-panel-tools").animate({top: "241px"}, 700);
        $(".Tutorial-controlZoom").animate({top: "172px"}, 700);
        $(".Tutorial-tracking").animate({top: "167px"}, 700)/
        $(".Tutorial-rightTools").animate({top: "92px"}, 700);
        $(".Tutorial-center").animate({top: "360px"}, 700);
        if (!MDM6API.ui.isMobile) {
            $(".Tutorial-tracking").addClass('hidden');
        }
        */
	},
    showInfoCE:function(){
    ...
    getStructureCE:function(){
	    ...
	    '<div id="'+this.idCE+'">'+
            ...
            '</div>'+
            '<div class="marker bounce"><div class="template_tutorialCE ttce_logo"></div></div>'+
	...
	loadCss:function(){
	    var obj = this;
	     $.when(
                    $('<link>', {rel: 'stylesheet',type: 'text/css',href: this.path+'tutorial.css?ver='+mdmVersion}).appendTo('head'),
