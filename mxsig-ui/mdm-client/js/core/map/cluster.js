define(['OpenLayers','request','dataSource','timer','marker','printControl'],function(OL,Request,DataSource,Timer,Marker,printControl) {
            ...
            success:function(data,extraFields){
                ...
                if(data){
                    if(data.response.success){
                        var options = {fColor:"none",lSize:2,lColor:"#01FCEF",lType:"line",type:'buffer',clearFeatures:true,layer:'SpetialPolygons'};
            ...
            success:function(data,extraFields){
                ...
                if(data){
                    if(data.response.success){
                        ...
                        /****/
                        if (!extraFields.lon) {
                            var coords = MDM6('getLonLatFromPixel',extraFields.xy.x,extraFields.xy.y);
                            extraFields.lon = coords.lon;
                            extraFields.lat = coords.lat;
                        }
                        var eventPanning = function(){
                            var newCenter = new OpenLayers.LonLat(extraFields.lon,extraFields.lat);
                        ...
                        var eventGetPxFronLonlat = function(lon,lat){
                                    var newCenter = new OpenLayers.LonLat(lon,lat);
                                    return MDM6('getPixelFromLonLat',newCenter);
                        }
                        
                        /****/
                        $("#panel-center").popup({getLastPosition:true,xy:extraFields.xy,title:'',text:div,lon:extraFields.lon,lat:extraFields.lat,btnClose:true,showOn:'now',events:{onMouseOut:evento},item:extraFields.item,eventPanning:eventPanning,eventGetPxFronLonlat:eventGetPxFronLonlat});
                        $("#"+idDiv).parent().attr('align','');
                ...
                if(data){
                    if(data.response.success){
                        if(data.data.size>1){
                            ...
                            if(data.data.size==1){
                                Marker.event({action:'delete',items:'all',type:'identify'});
                                var valueId = (data.data.children[0])?data.data.children[0].id:null;
                                setTimeout(function(){
                                    //showRecordCard({lonlat:extraFields.lonlat,xy:extraFields.xy});
                                    showRecordCard({id:valueId,lonlat:extraFields.lonlat,xy:extraFields.xy,item:extraFields.element,eventPanning:extraFields.eventPanning,eventGetPxFronLonlat:extraFields.eventGetPxFronLonlat,lon:extraFields.lon,lat:extraFields.lat});
                                },0);
                            }else{
                                extraFields.Event();
                            }
    ...
    var showRecordCard = function(a){
        ...
        var evento = (a.evento)?a.evento:$.noop;
        //requestRecordCard.setExtraFields({xy:coordinates,Event:evento,item:a.item,eventPanning:a.eventPanning,eventGetPxFronLonlat:a.eventGetPxFronLonlat,lon:a.lonlat.lon,lat:a.lonlat.lat});
	...
	if(a.whatshere){
        ...
        request.setExtraFields({xy:a.xy,lonlat:a.lonlat,Event:a.evento,eventPanning:a.eventPanning,eventGetPxFronLonlat:a.eventGetPxFronLonlat,lon:a.lonlat.lon,lat:a.lonlat.lat});
        request.execute();
	if(a.geometry){
