define(['OpenLayers','mapStyles','features','marker','georeference','popup','poi','modal','config','dataSource','request','notification','cluster','toolsConfig'],function(OL,mapStyle,Features,Marker,Georeference,Popup,Poi,Modal,config,dataSource,Request,Notification,Cluster,toolsConfig) {
	...
	*/
    ...
    var addExtra = function(map,layers,ctl,spetialLayer){
	Map = map;
	...
	Features.setSources(ctl,layers[0],Map,spetialLayer);
	//Features.init();
    ...
    var actionIdentify = function(e,lonlat,disableValidations,onlyIdentify){
			...
			if (!onlyIdentify) {
                if((Map.cluster.active)&&(!disableValidations)){
                    var eventoPanning = function(){
                            var newCenter = new OL.LonLat(lonlat.lon,lonlat.lat);
                            var newBounds = MDM6('calculateBounds',newCenter);
                            var sections = 5;
                            var decrementx =  (((newBounds.left*-1)-(newCenter.lon*-1))/sections);
                            var decrementy = (((newBounds.top-newCenter.lat)/sections));
                            
                            var Lonlat = new OL.LonLat(newBounds.left+decrementx,newBounds.top-decrementy);
                            MDM6('panTo',Lonlat);
                    }
                    var eventoGetPx = function(lon,lat){
                                var newCenter = new OL.LonLat(lon,lat);
                                return MDM6('getPixelFromLonLat',newCenter);
                    }
                    
                    if(Map.cluster.onlyDisplayRecordCard){
                        Cluster.showRecordCard({lonlat:lonlat,xy:{x:e.xy.x,y:e.xy.y},denue:false,evento:evento,eventPanning:eventoPanning,eventGetPxFronLonlat:eventoGetPx});
                    }else{
                        if(!Map.cluster.whatshere){
                            Cluster.show({xy:e.xy,lonlat:lonlat,evento:evento,geometry:Map.cluster.geometry,whatshere:Map.cluster.whatshere,eventPanning:eventoPanning,eventGetPxFronLonlat:eventoGetPx});
                        }else{
                            identifyWhatshere(e.xy);
                        }
                    }
                }else{
                    if ((Map.economic)||(Map.cenago)) {
                        if (Map.economic) {
                            amplify.publish( "identifyEconomic", {lon:lonlat.lon,lat:lonlat.lat,width:Map.getWidth(),resolution:Map.getResolution()} );
                        }
                        if (Map.cenago) {
                            var datos = $('#toolCenago').cenago('getVarsData');
                            var point = "POINT("+lonlat.lon+" "+lonlat.lat+")";
                            var paramsFind = {point:point,cvegeo:datos.geoSelected.join(','),level:datos.geoType};
                            var paramsCard = {
                                    "id":'',
                                    "level":datos.geoType,
                                    "point":point,
                                    ...
                                    "year":datos.year,
                                    "total":datos.showTotal
                            }
                            var evento = function(){
                                actionIdentify(null,{lon:lonlat.lon,lat:lonlat.lat},null,true);
                            }
                            
                            $('body').cardCenago({data:{find:paramsFind,card:paramsCard,title:'Censos de gobierno',event:evento,variable:datos.varActive.variable,connection:toolsConfig.cenago.dataSources,typeSelected:datos.typeVarSelection}});
                        }
                        //if (Map.eod) {
                        //    console.log(Map.onEODFeature(lonlat.lon,lonlat.lon));
                        //}
