...
define(['OpenLayers','config','dataSource','request'],function(OL,config,dataSource,Request) {
    ...
    var create = function(){
        ...
        if(insert){
            ...
            if(showPopup){
                MDM6('showPopupForMarker',marker[0].id,a.type,marker[0]);
            }
            return marker[0].id;
        }
    };
    ...
    var remove = function(){
        ...
        for(x in source){
            ...
            var selected = Map.getRegSelected();
            if ((selected)&&(selected.id==item)) {
                Map.clearPopup();
            }
        }
    };
    ...
    var getGeo = function(a){
        ...
        for(x in source){
            ...
            elemento.attributes['name']=convertToHtml(elemento.attributes.nom); 
