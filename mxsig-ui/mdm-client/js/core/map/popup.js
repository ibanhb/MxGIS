define(['OpenLayers','timer','cluster'],function(OL,Timer,Cluster) {
        ...
        show:function(){
            ...
            if(!obj.created){
                ...
                $("#PopupOL .close").click(function(){
                    ...
                    obj.effect('hide');
                    amplify.publish('clearRegSelected');
                });
            }
            if(!obj.visible){
        ...
        setData:function(){
            ...
            if(typeof(a.prev) == 'object'){
                var eventoPanning = function(){
                            var newCenter = new OL.LonLat(x,y);
                ...
                var eventoGetPx = function(lon,lat){
                 ...
                 if (a.prev.custom) {
                    ...
                    $("#panel-center").popup({item:a.element,event:evento,eventPanning:eventoPanning,eventGetPxFronLonlat:eventoGetPx,xy:a.px,lon:x,lat:y,title:a.title,text:a.prev.custom,btnClose:true,showOn:'now'});
                 }else{
                    if(a.prev.tabla.indexOf('denue')!=-1){
                           var lonlat = MDM6('getLonLatFromPrixel',a.px);
                           Cluster.showRecordCard({id:a.prev.id,lonlat:lonlat,xy:{x:a.px.x,y:a.px.y},item:a.element,eventPanning:eventoPanning,eventGetPxFronLonlat:eventoGetPx});
                    }else{
    ...
    $.widget( "custom.popup", {
                ...
                showPermitedSide:function(reposition){
                    ...
                    if (((!valid)&&(reposition))||((!valid)&&(obj.options.getLastPosition))) {
                          ...
                          this.options[key] = value;
                                    switch(key){
                                            ...
                                            case "getLastPosition":
                                                    this.options.getLastPosition = value;
                                                break;
