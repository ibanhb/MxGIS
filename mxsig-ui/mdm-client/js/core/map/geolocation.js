define(['OpenLayers','config','features','request','notification','../config/geolocation'],function(OL,config,Features,Request, Notification,dataGeolocation) {
    var control;
    var Layer;
    var demo=false;
    var points;
    ...
    var currentLocation=dataGeolocation.onError;
    ...
    var locateMe=function(e){
        ...
        var lon = (e.point)?e.point.x:e.lon;
        var lat = (e.point)?e.point.y:e.lat;
    ...
    var removeSpetialCharacter = function(a){
        var chain = a;
    ...
    var getCityCode = function(city){
        var locations = dataGeolocation.locations;
        var response = null;
        city = removeSpetialCharacter(city);
        for(var x in locations){
            var c = locations[x];
            var location = x+'';
            location = removeSpetialCharacter(location);
    ...
    var locateMeByApi = (function () {
        var onSuccess = function (geoipResponse) {
            var r = geoipResponse;
            var city = geoipResponse.city.names.en || 'your city';
            var region = geoipResponse.most_specific_subdivision.names.en;
            var lonlat = MDM6('toMercator',r.location.longitude,r.location.latitude);
            var params = {point:{x:lonlat.lon,y:lonlat.lat}};
            var cityCode = getCityCode(city);
            currentLocation = {lon:lonlat.lon,lat:lonlat.lat,city:city,region:region,cityCode:cityCode};
            if (paramsMap.params==null) {
                locateMe(params);
                amplify.publish( 'mapGeolocation',params);
            }
