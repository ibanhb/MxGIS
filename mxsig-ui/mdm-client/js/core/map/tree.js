define(['OpenLayers','tree','timer','linetime','dataSource'],function(OL,tree,timer,LineTime,DataSource) {//
    var Tree = {
        load:false,
        idItem:'treePruve',
        visible:false,
    ...
    var defineSource = function(){
        ...
        amplify.store('Wms',null);
        amplify.store('Windy',null);
        var L = getStored('Vectorial');
        var T = getStored('Text');
        var W = getStored('Wms');
        var Windy = getStored('Windy');
        if(!L){
            for(x in tree.layers.groups){
                var g = tree.layers.groups[x];
                for(y in g.layers){
                    ...
                    if(y.indexOf('c')!=-1){
                        ...
                        if (l.format) {
                            if (l.active) {
                                var f= l.format.toLowerCase();
                                switch (f) {
                                    case 'wms':
                                        W=storeItem(W,l.position,y,x,l.scale);
                                        break;
                                }
                            }
                        }else if (l.windy) {
                                if (l.active) {
                                    Windy=storeItem(Windy,l.position,y,x,l.scale);
                                }
                        }else{
                            if(l.active){
            ...
            if(W!=null){
                storeInTem('Wms',W);
                Tree.layerAfected['Wms']=true;
            }
            if(Windy!=null){
                storeInTem('Windy',Windy);
                Tree.layerAfected['Windy']=true;
            }
        }else{
            storeInTem('Vectorial',L);
            if(T){
    ...
    var clearTempStore = function(){
        Tree.tempStore={Text:{item:{},position:{}},Vectorial:{item:{},position:{}},Wms:{item:{},position:{}},Windy:{item:{},position:{}}};
    };
    var reset = function(){
        clearTempStore();
        Tree.timer.execute();
    };
    var getLayer = function(i,data){
        var fi = i.substring(0,1);
        fi = fi.toLowerCase();
        var type='';
        switch(fi){
            case 'c':type='Vectorial';
                    break;
            default: type='Text';
        }
        if (data) {
            if (data.format) {
                var f = data.format.toLowerCase();
                switch (f) {
                    case 'wms':
                        type='Wms';
                }
   ...
   */
    ...
    var addToRepository = function(e){
        ...
        for(var x=0;x<e.length;x++){
            var i = e[x];
            if (i.id=='cenago') {
                DataSource.mapEvaluation = (i.active)?1:0;
            }
            if(r[i.id]){
            ...
            if (i.format) {
                r[i.id].format = i.format.toLowerCase();
                r[i.id].url = i.url;
                r[i.id].image = i.image;
                r[i.id].tiled = (i.tiled)?i.tiled:false; 
            }
            if (i.windy) {
                r[i.id].windy=i.windy;
            }
            Tree.layerAfected[getLayer(i.id,i)]=true;
