...
define(['OpenLayers','config','timer','marker','popup','request','modal','dataSource','notification','cluster','escuelas'],function(OL,config,Timer,Markers,Popup,Request,Modal,dataSource,Notification,Cluster,Escuelas) {//
    ...
    var getDistance = function(){
        var f = arguments[0];
        var getMeasure = (f.custom.measure)?false:true;
        if (getMeasure) {
        ...
        if(t=='cluster'){
            ...
            if (f.custom) {
                if(f.custom.getArea){
                    ...
                    params['title']='<div style="min-width:70px;">'+f.attributes.name+'</div>';
                    params['element']='point';
                    params['prev']=((f.attributes.desc)?f.attributes.desc:'');
                    params['event']=null;
            }
    ...
    var showInfo = function(){
        ...
        switch(reg.selected.type){
            case 'eod':
                type='eod';
                var f = Map.getEODFeature(reg.selected.id);
                break;
            case 'poi':
            case 'identify':
            case 'search':
    ...
    var showPopup = function(id,type,usePoint){
    ...
    MDM6('define','showPopupForMarker',function(id,type){
        showPopup(id,type,true);
        });
        ...
        controls.Editor = new OL.Control.ModifyFeature(
                            layers[0],{
                                onModificationEnd:function(e){
                                        ...
                                        if((e.custom.item=="feature")&&(e.custom.type=="georeference")&&(e.custom.store)){
                                            storeFeature(e);
                                        }
                                },
                            ...
                            layers,
                            {
                                ...
                                onSelect: function(e){
                                    setTimeout(function(){
                                        ...
                                        if (e.layer.name=="hoverPolygons") {
                                            e.attributes.fColor=c.filled;
                                            e.attributes.lColor=c.line;
                                            e.attributes.lSize=c.size
                                            e.layer.drawFeature(e);
                                            MDM6('onDisplayFeatureMouseOver',e.attributes.data);
                                        }else{
                                            if(e.cluster){
                                                if(e.cluster.length==1){
                                                ...
                                                if ((e.attributes.viajes)||(e.attributes.viajes==0)) {
                                                    reg.setSelected(e.id,'eod','point');
                                                }else{
                                ...
                                onUnselect:function(e){
                                        ...
                                        if (e.layer.name=="hoverPolygons") {
                                            e.attributes.fColor='transparent';
                                            e.attributes.lColor='transparent';
                                            e.layer.drawFeature(e);               
                                        }
                                }
                            }
                        );
        controls.Hover.handlers.feature.stopDown = false;
        return controls;
    };
    ...
    var createSpetial = function(){
        var a = arguments;
        var data = getFeatureFromWKT(a[0].wkt);
        if(data.valid){
            for(var x in data.features){
                var feature = data.features[x];
                feature.attributes = a[0].params;
            }
            SpetialLayer.addFeatures(data.features);
            SpetialLayer.redraw();
        }else{
    ...
    var create = function(){
        ...
        if(data.valid){
            ...
            var isPoint = (a[0].wkt.indexOf('POINT')!=-1)?true:false;
            var Measure = (!isPoint)? ((a[0].params.fColor)?getArea(lastFeature,false):getDistance(lastFeature,false)):'0';
            ...
            if(a[0].params.type=='georeference'){
                ...
                var feature = reg.get(params.id);
                var dataFeature = $.extend({}, params,{wkt:feature.geometry+''});
                MDM6('onDrawFeature',dataFeature);
            }
    ...
    $('#map').bind('contextmenu', function(e){
            ...
            var i = reg.selected;
                    ...
                    if(i.id){
                var valid = (i.item=='feature')?true:false;
				if((Map.onMap)&&(valid)){
                                var f = (i.item=='feature')?reg.data[i.id]:Markers.reg.get(i.id);                  
    ...
    var removeSpetial = function(){
        var id = arguments[0];
        SpetialLayer.removeFeatures(SpetialLayer.getFeatureById(id));
    }
    var remove = function(){
        var id = arguments[0];
        var f = reg.get(id);
    ...
    var addBufferToBuffer = function(id,size,type,replace,LonLat){
        ...
        if(type=='point'){
            ...
            if(f){
                ...
                //console.log(config);
                addBuffer.setParams(JSON.stringify({id:f.custom.db,size:size,project:config.dataSource.proyName }));
                addBuffer.execute();
            }
        }
    };
    var addArea = function(id,table,size){//{id:'',table:'',size:''}
        ...
        //console.log(config);
        addBuffer.setParams(JSON.stringify({id:params.id,table:params.table,size:params.size,project:config.dataSource.proyName}));
        addBuffer.execute();
    };
    ...
    var locateAndZoom = function(){
        ...
        MDM6('zoomToExtent','Mapc',f.geometry.bounds);
    };
    ...
    var importer = function(opc,data,typeFile){
        ...
        var readFormat = function(text,typeFile){
            ...
            if(totalPolygons.length>0){
                ...
                //for(x in resp ){
                    var extra = totalTemporalGeoParams.concat(resp);
...
*/
    ...
    var isGeoJson = function(data){
        ...
        try {
            var format = new OpenLayers.Format.GeoJSON(/*in_options*/);
            var features = format.read(data);
            response = (features.length>0)?true:false;
        } catch(e) {
            response=false;
        }
        return response;
    ...
    return {
        ...
        addSpetialFeature:function(wkt,params){
            ...
            if (params.layer) {
                createSpetial({wkt:wkt,params:params});
            }else{
                create({wkt:wkt,store:false,params:params});
            }
        },
        deleteSpetialFeature:function(layer){
            if (layer) {
                SpetialLayer.destroyFeatures();
            }else{
                var features = reg.type['buffer'];
