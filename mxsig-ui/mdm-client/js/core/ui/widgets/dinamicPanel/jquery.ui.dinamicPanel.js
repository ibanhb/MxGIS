$.widget("custom.dinamicPanel", {
     ...
     searchTimer:0,
     options:{
		 canShowDetail:function(){return true},
         bufferLayers:[],
		 treeLayers:null,
         recurrentIdentify:[],
         layers:[],
		 ...
		 controlsMap:null, // configuración de los controles del mapa
		 onAnalysis:function(){},
	 ...
	 setCanShowDetail:function(func){
		var obj = this;
		obj.options.canShowDetail = func;
	 },
     ...
     createLeyendList:function(){
		...
		var baseMap = obj.options.getCurrentBaseMap();
		var isBaseUrl =  !(baseMap.legendUrl === undefined);
		...
		if(!isBaseUrl){ //si no hay url personalizada para mapa base
		//incluye capas del mapa base para leyenda 
		...
		//mapa base tiene url personalizada? añade sus capas con la url personalizada
		if(isBaseUrl){
			for(var x in baseLayers){
				cadena+= '<img src="'+baseMap.legendUrl+x+'"/>';
				obj.currentLegends.push(baseMap.legendUrl+x);
			}
		}
		//------------------------------------
     ...
     showPanelResult:function(){
        ...
        if (panel.html() != ''){
            if (results.attr('visible') == 'false'){
                //results.fadeIn('fast');
                results.attr('visible',true).show();
            };
        }else{
             if (results.attr('visible') == 'true'){
                results.attr('visible',false).hide();
	 ...
	 showSlideBtn:function(){
		var obj = this;
		$('#'+obj.id+'_hideSearch').fadeIn('slow');
		 
	 },
	 hideSlideBtn:function(){
		var obj = this;
		$('#'+obj.id+'_hideSearch').fadeOut('slow');
	 },
     hiddeTools:function(){
        var obj = this;
     ...
     panelTransparency:function(opc){
        ...
        if (opc === undefined){
            ...
            if ($('#'+obj.id+'_detail').html() == '')
            	obj.showPanelResult();
				obj.showSlideBtn();  //muestra boton de slide  
            ...
            if (!ban && !obj.mouseOnPanel){
                ...
                obj.hiddeResults();
				obj.hideSlideBtn();  //ocultaboton de slide  
     ...
     createPanel:function(type,data){
        ...
        switch (type) {
            ...
            case 'geo':
		    		...
		    		cadena+= '                  <strong>Archivo no válido</strong>';
                    cadena+= '              </p>';
                    cadena+= '          </div>';
                    cadena+= '      </div>';
                    cadena+= '   </div>';
        ...
        var showList = function(){
            ...
            $('.dinamicPanel-featureEditName').each(function(){
                $(this).click(function(){
                   ...
                   $(this).css('display','none');
                   $('.dinamicPanel-featureOkName[idref="'+idref+'"]').css('display','').click(function(event){
                        var idRef = $(this).attr('idref');
                        var item = getPoly(idRef);
                        item.data.name = $('#inputText_'+idRef).val();
                        editPoly(item);
                        map.Feature.event({select:true,id:item.id});
     ...
     showBufferList:function(){
        ...
        $('#'+obj.id+'_buffer_result button[action="selection"]').each(function(){
            $(this).click(function(){
               var idref = $(this).attr('idref');
        ...
        if (!(poly === undefined) && layer != '-1'){
            ...
            var printDetailHeader = function(data){
                ...
                if (!(totales === undefined) && totales != null){
                    ...
                    for (var x in list){
                        var item = list[x];
                        cadena+= '<div class="customDetail-header-item"><label>'+item.label+'</label><span>'+parseInt(item.value,10).format()+'</span></div>';
                    };
                }
				if (!value)
			...
			var headerHeight = $('#main').position().top;
            var winHeight = $(window).height()-(headerHeight + obj.element.position().top + 320);
			winHeight = (winHeight > 420.5)?420.5:winHeight;
            var detalle = {};
                ...
                var content = '<div class="dinamicPanel-customDetail" style="height:'+winHeight+'px;">';
					...
					content+= '		</div>';
					content+= '     <div class="dinamicPanel-customDetail-container" style="height:'+winHeight+'px;position:relative !important;">';
                ...
                detalle.onClose = function(){obj.createPanel('buffer');obj.options.onAnalysisClose()};
                detalle.onCreate = function(){
					...
					//evento previo a analisis
					obj.options.onAnalysis(params);
			...
			$('.dinamicPanel-customDetail-container').css('position','relative');
			
			/* Ajuste de botones */
			$('#dinamicPanel_buffer_header').append(
     ...
     createPolyList:function(list){
        ...
        var getItemTools = function(item,pos){
            ...
            var cadena = '';
            for (var x in tools){
                cadena+='<button idref="'+item.id+'" pos="'+pos+'" alt="'+labels[tools[x]]+'" title="'+labels[tools[x]]+'" action="'+tools[x]+'" type="'+tools[x]+'" class="dinamicPanel-featureBtn dinamicPanel-sprite dinamicPanel-'+tools[x]+'"></button>'; 
            };
            return cadena;
        };
        var cadena = '';
    ...
    getAbstract:function(data){
     ...
     openToolPanel:function(size){
        ...
        $('#'+obj.id+'_toolSet').animate({'height':height+'px'},function(){
     ...
     closeToolPanel:function(){
        var obj = this;
        $('#'+obj.id+'_toolSet').animate({'height':'55px'},function(){
			obj.adjustResultHeight();
		});
        obj.showPanel();
     },
	 ...
	 exportDetail:function(data,type){
		 var obj = this;
		 var title = data.data.table.substring(0,data.data.table.length-3);
		 
		 var cols = [];
		 var values = [];
		 
		 var list = data.data.list;
		 cols.push({label:'Campo'},{label:'Valor'});
		 for(var x in list){
			 var item = list[x];
			 values.push([item.name,item.value.substring(7,item.value.length-8)]);
		 }
		 
		 if(list)
			obj.exportList(title,cols,values,type);	 
	 ...
	 exportList:function(title,cols,list,type){
		...
		obj.getData(dataSource,params,function(dataRes){ 
			if(dataRes.response.success){
				...
				var url = dataSource.urlGet+'/'+type+'/'+id;
				window.location.assign(url);
			}
		});
     ...
     //Identify Recive ----------------------------------------------------
     //--------------------------------------------------------------------
	 reverseTranslateLayerByZoom:function(layer){
		 var obj = this;
		 var r = layer;
		 if(layer.substr(layer.length-2,1) == '_'){
			 var item = obj.options.getLayer(layer.substr(0,layer.length-2)); 
			 if(item)
				 r = item.idLayer;
		 }
		 return r;
	 },
	 translateLayerByZoom:function(layer){
		 var obj = this;
		 var item = obj.options.getLayer(layer);
		 var zoom = obj.options.map.getZoomLevel();
		 var r = layer;
		if(item && item.dinamicNameOnZoom && item.dinamicNameOnZoom['z_'+zoom]){
			r+='_'+item.dinamicNameOnZoom['z_'+zoom];
		}
		return r;
	 },
     identifyPoint:function(data,byText){
        var obj = this;
		...
		if(!obj.options.controlsMap.identify.fixedLayer){
			...
			blayers = obj.options.controlsMap.identify.fixedLayer;
			var activeLayers = blayers;
		}
		
		//check dinamic names per Zoom
		var tactive = activeLayers.split(',');
		for(var x in tactive){
			tactive[x] = obj.translateLayerByZoom(tactive[x]);
		}
		activeLayers = tactive.join();
		...
		var pointIdentify = {lon:data.lon,lat:data.lat}; 
        var params = { //Params----------------------
            'tables':activeLayers,
            'project':proyName,
            'x':data.lon,
		...
		function(dataRes){ //success
			if (!(dataRes === undefined)){
				...
				if (success && tables && tables.length > 0){
					...
					var recCount = 0;
					var tabCount = 0;
					var point = obj.options.map.transformToGeographic(data.lon,data.lat);
					var pointDegree = obj.options.map.transformToDegrees(data.lon,data.lat);
					var dataObject = {
							id:'Informaci&oacute;n del punto:',
							name:'<div style="font-size:85%;float:left;clear:both">'+pointDegree.lon+', '+pointDegree.lat+'</div>'+
								 '<div style="font-size:85%;float:left;clear:both">'+point.lon.toFixed(15)+', '+point.lat.toFixed(15)+'</div>'+
								 '<div style="font-size:85%;float:left;clear:both" id="dinamicPanel_point_altimetry" point="'+data.lon+','+data.lat+'">Altura:Cargando...</div>'
					};
					var listTables = [];
					for (var t in tables){
						tabCount++;
						...
						tabla.point = pointIdentify;
						var results = tables[t].elements;
						var idTabla = tables[t].alias;
						var tablaRecord = [];
						var wkt = null;
						var download = [];
						if(!firstTable)firstTable = idTabla;
						...
						for (var r in results){
							...
							for (i in fields){
								...
								var type = nameField.substr(0,3);
								var canDisplay = (['(d)','(s)','(i)','(h)','(t)'].indexOf(type) >= 0); //download, show, image
								if(canDisplay){
									nameField = obj.stringAsType(nameField.substr(3));
									if(type == '(d)'){
										download.push({label:fields[i].label.substr(3),value:fields[i].value});
									}else{
										record[nameField]=fields[i].label.substr(3)+'|'+fields[i].value;
									}
								}else{
									nameField = (nameField == 'wkt' || nameField =='tabla' || nameField =='id' || nameField =='nombre')?nameField:'value';
									record[nameField]=fields[i].value;
									if(nameField == 'wkt')
										wkt =  fields[i].value.value;
								}
							};
					...
					if(obj.options.canShowDetail(obj.options.getActiveLayers())){
						var firstTable = obj.reverseTranslateLayerByZoom(firstTable);
						var infoLayer = obj.options.getLayer(firstTable);
						if(infoLayer && infoLayer.displayOnMarker){
							obj.displayOnMarker(dataObject);	
						}else{
							obj.showDetail(dataObject,'identify',null,byText);
						}
					}
					
					obj.options.onIdentify(dataObject);
				}else{
     ...
     searchDetail:function(id,table){
        var obj = this;
        var dataSource = $.extend({},obj.options.dataSource.identifyDetail);
		var specialTables = obj.options.dataSource.detailTableSpecial;
		
		if(specialTables.indexOf(table) >= 0){
			table = table+'_sh';
		}
	 ...
	 displayOnMarker:function(info){
		 	var obj = this;
		 	var position = (info.list && info.list.length > 0)?info.list[0].point:null;
		 	var list = info.list[0].list[0];
		 
		 	var image = (list.imagen)?list.imagen.split('|')[1]:null;
		 	var pdf = (image)?image.substr(0,(image.length-3))+'pdf':null;
		 	var id = list.id;
		 	var table = list.tabla;
		 	var wkt = list.wkt;
			var download = (list.download)?list.download:null;
		 	var ficha = (list.ficha)?list.ficha.split('|')[1]:null;
		 	var detalle = (list.detalle)?list.detalle.split('|')[1]:null;
		 
		 	if(wkt){
				MDM6('deletePolygons','SpetialPolygons'); 
				var options = {fColor:"none",lSize:2,lColor:"#66C5CC",lType:"line",type:'buffer',clearFeatures:true,layer:'SpetialPolygons'};
				MDM6('addPolygon',wkt,options);
			}
			...
			delete list.tabla;
			delete list.wkt;
		 	delete list.download;
		 	delete list.ficha;
		 	delete list.detalle;
		 
			var cadena = '<div id="dinamicPanel_markDetail_container" class="dinamicPanel-markDetail-container">';
		 		
		 		if(image)
		 		cadena+= '	<a href="'+pdf+'" target="_blank"><div class="dinamicPanel-markDetail-image" style="background-image:url('+image+');"></div></a>';		
		 
		 		cadena+= '	<div class="dinamicPanel-markDetail-content">';		
		 
		 		if(ficha) cadena+= ficha;
		 		/*cadena+= '	<table class="mdm-table-autoColor">';		
		 		for(var x in list ){
					var item = {label:list[x].split('|')[0],value:list[x].split('|')[1]};
					cadena+= '	<tr><td>'+item.label+'</td><td align="right">'+item.value+'</td></tr>';		
				}
		 		for(var x in download){
					var item = download[x];
					cadena+= '	<tr><td colspan="2" align="right"><a href="'+item.value+'" target="_blank">'+item.label+'</a></td></tr>';		
				}
		 		cadena+='	</table>'*/
		 		cadena+= '</div>';
		 
		 		if(detalle)
		 		cadena+= '<div id="dinamicPanel_markDetail_btn" class="dinamicPanel-markDetail-btn">Ver detalle</div>';
		 
			setTimeout(function(){
				obj.options.markerWithText(position,cadena,function(){
					$('#dinamicPanel_markDetail_btn').click(function(){
						obj.options.systemMessage(detalle,{width:450,height:350});
						
						setTimeout(function(){
							$('.service-container-title').each(function(){
								$(this).click(function(){
									var parent = $(this).parent();
									var open = parent.attr('_open');
									parent.attr('_open',!(open == 'true'));
								});
							});
						},500);
     ...
     showDetail:function(pos,type,hidde,byText){
        var obj = this;
		obj.clearSearch(((byText)?true:false));
		//Evento en Modulos
        ...
        if (type == 'content'){ //este boque solo se usa para sustituciones completas
            ...
            if (!(pos === undefined)){
				...
				if(id.substring(0,15) == '<!DOCTYPE html>')
					showDownload = false;
                ...
                var cadena = '<div class="dinamicPanel-detailContent">';
                    ...
                    if (!(list === undefined)){
                        ...
                        var mainLabel = '';
                        var prefijo = '';
						var isMultiple = (list[0] && list[0].list && list[0].list.length > 1);
						$('#'+obj.id+'_detail').attr('ismultiple',isMultiple);
                        for (var x in list){
                            if (type == 'identify'){
								$('#'+obj.id+'_detail').attr('isidentify','true');
								...
								$('#'+obj.id+'_detail').attr('isidentify','false');
                                var item = list[x];
                                var tcadena = '';
                                ...
                                //si el valor es una imágen reemplaza para presentarla
                                if(item.name == '_image'){
                                    cadena+='<div class="dinamicPanel-detailList-item"><image src="'+item.value+'" width="100%" /></div>';    
                                }else{
                        ...
                        if (type == 'identify'){
                            ...
                            cadena+='       <div class="dinamicPanel-detailMainLabel"><label>'+prefijo+'</label>'+mainLabel+'</div>';
					...
					if (type == 'identifyDeep' && showDownload){
						obj.lastDetailResults =  {data:pos,type:type};
						
						cadena+='<div class="dinamicPanel-detailList-exportTools">';
						cadena+='	<div class="dinamicPanel-sprite dinamicPanel-xls-short" id="dinamicPanel_exportDetail_xls"></div>';
						cadena+=' 	<div class="dinamicPanel-sprite dinamicPanel-csv-short" id="dinamicPanel_exportDetail_csv"></div>';
						cadena+='</div>';
					}
					...
					$('#dinamicPanel_exportDetail_xls').click(function(e){
						obj.exportDetail(obj.lastDetailResults,'xls')
						e.stopPropagation();
					});
					$('#dinamicPanel_exportDetail_csv').click(function(e){
						obj.exportDetail(obj.lastDetailResults,'csv')
						e.stopPropagation();
					});
				...
				//Reviza si debe cargar altimetria de punto
				if($('#dinamicPanel_point_altimetry').attr('id')){
					obj.loadAltimetryToIdentify($('#dinamicPanel_point_altimetry'));
				}
				
            };
        };
     },
	 loadAltimetryToIdentify:function(container){
		var obj = this;
		var point = container.attr('point').split(',');
		var ds = $.extend({},obj.options.dataSource.mousePosition.elevation);
		ds.contentType = "application/json; charset=utf-8";
		var params = JSON.stringify({x:point[0],y:point[1]}); 
        obj.getData(ds,params, 
        function(res){ //success
			if(res.data && res.data.elevation){
            	var value = res.data.elevation;
				container.html('Altura:'+value.format()+' metros.')
			}
        });
	 },
     //--------------------------------------------------------------------
    ...
    //--------------------------------------------------------------------
     assignSearchOnLink:function(){
 	 ...
 	 hideSearch:function(){
		var obj = this;
		obj.element.animate({left:-450},function(){
			obj.element.attr('isHide','true');
			$('#'+obj.id+'_showSearch').css({left:'400px'}).animate({left:'450px'},500);
		});
	 },
	 showSearch:function(){
		var obj = this;
		$('#'+obj.id+'_showSearch').fadeOut('slow',function(){
			$('#'+obj.id+'_showSearch').css({left:'-60px'}); 
			obj.element.animate({left:'0px'});
			obj.element.attr('isHide','false');
		});
	 },
     initSearchCoords:function(coord){
        //Modificado por Manuel
     	var obj = this;
        var lonlat = obj.options.map.intersectExtent(coord[0],coord[1]);
        if (!lonlat) {
            lonlat = obj.options.map.transformToMercator(coord[0],coord[1]);
            lonlat = obj.options.map.intersectExtent(lonlat.lon,lonlat.lat);
        }
     	if (
     	      lonlat 
     	){
     		obj.identifyPoint(lonlat,'byText');
			...
			obj.mapNewMarker('coordenada: '+coord[0]+','+coord[1],
     		...
     		obj.mapExtend([lonlat.lon,lonlat.lat]); 
	...
	//Fin Manuel
     ...
     initSearchCross:function(text,func){
     	var obj = this;
        ...
        function(data){ //success
            ...
            if (!(data === undefined) && data.response.success){
                ...
                if($.isFunction(func)){ //busqueda canalizada a funcion
                    func(object);
                }else{
	 ...
	 privateSearch:function(search){
		var obj = this;
        var type = obj.getSearchType(search.text);
        if(type == 'cross'){
            obj.initSearchCross(search.text,search.func)
        }else{
	...
	searchNotFound:function(search){
		 var obj = this;
		 search = (search)?'No se encontraron resultados para <b>'+search+'</b>':'No se encontr&oacute; informaci&oacute;n';
     ...
     _create: function() {
         ...
         .addClass("custom-dinamicPanel no-print").attr('isHide','false');
     ...
     adjustResultHeight:function(height){
		...
		var posTop =  $('#mdm6DinamicPanel_results').position().top;
		var rheight = $('#panel-center').height();

		rheight = (rheight-posTop)-90;

        height = (height === undefined)?$(window).height():height;
        //if (height < 750){
        $('#'+obj.id+'_results').css('max-height',(rheight)+'px').css('overflow-y','auto');    

		 //$('#'+obj.id+'_results').animate({'max-height':rheight+'px'}).css('overflow-y','auto'); 
        //}else{
        //    $('#'+obj.id+'_results').css('max-height','').css('overflow-y','');    ;     
        //};
		//$('.dinamicPanel-detailPoint').css('max-height',(rheight-100)+'px');
		$('.dinamicPanel-detail').css('max-height',(rheight-100)+'px');
     ...
     _refresh: function() {
        ...
        var cadena = '<div id="'+obj.id+'_search" class="dinamicPanel-search dinamicPanel-border dinamicPanel-shadow dinamicPanel-box-sizing">';
            cadena+= '  <div id="'+obj.id+'_showSearch" class="dinamicPanel-sliderightbtn dinamicPanel-showSearch dinamicPanel-shadow" style="display:none"></div>';
		 	cadena+= '  <div id="'+obj.id+'_hideSearch" class="dinamicPanel-slidebtn dinamicPanel-hideSearch dinamicPanel-shadow" style="display:none"></div>';
            ...
            obj.adjustResultHeight();
		 	...
		 	$('#'+obj.id+'_hideSearch').click(function(){
				obj.hideSearch();
			});
		 	$('#'+obj.id+'_showSearch').click(function(){
				obj.showSearch();
