define(function(){
	var module = 
		   {
			pathType:['free','suggested','optimal'],
				...
				//inicia el spinner de proceso
					...
					for(var x in lastList){
						if (nameProcess == lastList[x].id){
							...
							var t_type = type; //(type == 'free')?'suggested':type; //ajuste necesario por movimiento en rutas
							var pathType = $.inArray(t_type,obj.pathType); //tipo de ruta 
			...
			showUniqueRoute:function(type){
				var obj = this;
				//inverted
				var typesConversor = {'optimal':'free','free':'optimal','suggested':'suggested'};
					...
					for(var x in listRoutes){
						...
						if(route.status){ //si el status de la ruta es procesada, y aun no ha sido impresa
							...
							for(var y in list){
								...
								route.length+= segment.value.long_km;
								route.time+= segment.value.tiempo_min;
								var geo = segment.value.geojson;
								obj.map.Routing.event({action:'add',geometry:geo,type:typesConversor[type],params:segment.value});
							}
						}
					}
			...
			printItinerary:function(type){
				...
				if(list.length > 0){
					...
					var mrkPoint = function(item){
						...
						point = obj.map.transformToDegrees(point[0],point[1]);
			...
			createItinerary:function(type){
				...
				//inicia el spinner de proceso
					...
					*/		
							type = type;//(type == 'free')?'suggested':type;
							var pathType = $.inArray(type,obj.pathType); //tipo de ruta 
							var currentConfig = obj.configData.currentSelection;
							...
							obj.sakbe.routing('detail',pathList[index],pathList[index+1],pathType,vehicle,currentConfig.extraAxle,null,null, //null block lines, scale
								function(data){
									...
									type =type; // (type == 'suggested')?'free':type;
									obj.routeItenerary.current[type].typeRoute = type;
			...
			saveStats:function(data){
				...
				var dataSource = $.extend(true,{},obj.main.options.dataSource.saveStats);
