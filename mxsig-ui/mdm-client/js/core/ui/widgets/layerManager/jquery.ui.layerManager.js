 ...
 $.widget("custom.layerManager", {
     ...
     openRightCarrucel:function(){
        ...
        if (obj.enableRC){
			...
			obj.options.onOpenRightCarrucel();
        }
     ...
     closeRightCarrucel:function(){
        ...
        if (obj.enableRC){
			...
			obj.options.onCloseRightCarrucel();
        }
     },
     ...
     rightCarrucel:function(){
        ...
        var cadena = '<div id="'+obj.id+'_v_c_slider" style="top:0px;height:'+height+'px;" class="layerManager_v_c_slider">';
            for (var x in data){
				var hide = (data[x].hidden)?' style="display:none" ':'';
                cadena+= '<div idref="'+x+'" class="layerManager_v_c_item layerManager-box-sizing" '+hide+'>';
	 ...
	 getCurrentBase:function(){
		 var obj = this;
		 return (obj.options.baseLayers[obj.currentBase]);
	 },
     //Custom functions----------------------------------------------------
	 ...
	 selectTheme:function(id){
		...
		if(layers.wktExtent && layers.wktExtent != ''){
			obj.options.gotoExtend(layers.wktExtent);
		}
     ...
     _create: function() {
         ...
         var cadena = '<div id="'+obj.id+'_h_container" class="layerManager-h-container layerManager-shadow-n">';
             ...
             cadena+= '     <div id="'+obj.id+'_layerManager_btnCollapse_" class="layerManager-btnCollapse_c layerManager_btnCollapse_set">';
             cadena+= '         <span class="layerManager-sprite layerManager-iconCollapse"></span>';
             cadena+= '     </div>';
