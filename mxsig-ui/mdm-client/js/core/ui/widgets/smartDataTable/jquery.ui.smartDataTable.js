/*
 Autor:Jos� de Jesus Loera Velasco
 
 Ejemplo de uso:
 var usuario = {user:'01',subUser:'02',edo:'01',action:'idle'};
    
  $(document).ready(function(){
    $( "#tabla" ).smartDataTable({
                  searchParams:{urlData:'http://10.28.20.77:8080/TableAliasCE/GetAgebsCE'},
                  extraParams:usuario,
                  buttons:[{title:'Eliminar',sprite:'ui-icon ui-icon-close',action:'remove'},
                           {title:'Ver',sprite:'ui-icon ui-icon-search',action:'view'}],
                  buttonAction:function(id,action){alert('llego esto '+id+' '+action)}
                });
  });
...
*data: {value:[{fields:[{label:_id, value:01001}, {label:id, value:01001}, {label:_hidden_extent,�}]},�],�}
 currentPage: 1
 numberOfPages: 2
 pageSize: 20
 recordCount: 11
 time: 1378487189
 **value: [{fields:[{label:_id, value:01001}, {label:id, value:01001}, {label:_hidden_extent,�}]},�]
*response: {success:true, message:null}
 message: null
 success: true
 
------------------------------------------------------------------------ 
 para mas configuraciones analizar el objeto options del widget
...
$(function() {
    ...
    $.widget( "custom.smartDataTable", {
      ...
      options: {
		...
		convertRequest:null, //convierte peticion antes de enviarla
		jsonXhrFields: null,
        //functions
	  ...
	  setInputFilter:function(){
		  var obj = this;
		  var input = $('#'+obj.element.attr('id')+'_filterInput');
		  var btn = $('#'+obj.element.attr('id')+'_filterBtn');
		  var btnClear = $('#'+obj.element.attr('id')+'_filterBtnClear');
		  ...
		  input.keypress(function(event) {
				if ((event.which == 46) || (event.which < 48 || event.which > 57)) {
			  	...
			  	obj.options.filter.value = input.val();
				console.log(key);
				if(key.keyCode == 13){
					btn.click();		
				}
		  ...
		  btn.click(function(){
					if(input.val().length >= obj.options.filter.chartoFilter){
						obj.options.filter.isOn=true;
						obj.initSearch();
					}else{
						if (obj.options.filter.isOn){
							obj.options.filter.isOn = false;
							obj.initSearch();
						}
					}	
		  });
		  btnClear.click(function(){
				obj.options.filter.value = '';
				obj.options.filter.isOn = false;
				obj.initSearch();
		  });
	  },
	  getParams:function(){
		...
		//si existe el filtro
		if(obj.options.filter.active)
			params[obj.options.filter.field] = (obj.options.filter.isOn)?obj.options.filter.value:'';	
	  ...
	  createPagination:function(data){
		...
		var rcadena = '<div style="position:absolute;left:5px;right:5px;bottom:6px;text-align:center;display:block; height:20px;">';
			rcadena+= '	<span id="paginationSearch" numPages="'+numPages+'">'+prevSet+cadena+nextSet+'</span>';
			if(obj.options.filter.active){
				rcadena+='	<div id="'+idElement+'_filter" class="smartTable-filterContainer" filter="'+((obj.options.filter.isOn)?'true':'false')+'">';
				rcadena+='		<input id="'+idElement+'_filterInput" class="smartTable-filterInput" type="text" placeholder="Filtro" value="'+((obj.options.filter.value)?obj.options.filter.value:'')+'" />';
				rcadena+='		<span id="'+idElement+'_filterBtn" class="smartTable-filterBtn ui-icon ui-icon-search"></span>';
				rcadena+='		<span id="'+idElement+'_filterBtnClear" class="smartTable-filterBtnClear ui-icon ui-icon-circle-close"></span>';
				rcadena+='	</div>';
			}
			rcadena+= '</div>';
		//
		...
		if (field.label != '_id'){
			...
			switch (tipo){
				case 'btn':
					...
					break;
				case 'avatar':
					  result = {type:'avatar',value:field.value};
					break; 
				case 'hidden':
			...
			if (records.length > 0){
				for (var x = 0; x < records.length; x++){
					...
					if (obj.options.displayType == 'list' && (obj.options.buttons.length > 0 || incomeButtons.length > 0 )  ){
						...
						if(avatar != ''){
							field+= '<div class="smartTableField-avatar smartTableField-avatar-'+avatar+'"></div>';
						}
					}else{
						field= '<div>'+field+'</div>';
					}
					
					var hasAvatar = (avatar != '')? ' smartDataTable-avatar':'';
					var registry = (obj.options.displayType == 'list')?'<div idref="'+id+'" '+attr+' class="'+obj.element.attr('id')+'_record '+obj.options.tableType.tableStyleClass+hasAvatar+'">'+field+'</div>':field;
					cadena+= registry;
				...
				cadena=(obj.options.displayType != 'list')?'<table class="'+obj.options.tableType.tableStyleClass+hasAvatar+'">'+headers+'<tbody>'+cadena+'</tbody></table>':cadena;
			}
      ...
      getData:function(url,params){
            ...
            var id = obj.element.attr('id');
			var jsonObj = {
					  contentType : obj.options.contentType ||  "application/json; charset=UTF-8",
				   ...
				   if(obj.options.convertRequest && $.isFunction(obj.options.convertRequest))
				   	jsonObj = obj.options.convertRequest(jsonObj);
