$.widget("custom.infoCard", {
	...
	addGeo:function(data){
		...
		if((obj.currentData.status=='geo')&&(obj.currentData.edoList.length<=12)){
			...
			if(!obj.existGeo(data.cvegeo)){
                ...
                var nivel=vals.geoType;
                var level=vals.selection.filter.toLowerCase();
                var national=false;
                
                if (nivel=="ZM") {
                    level="zm";
                    national=(vals.geo.id == "9000")?true:false;
                }
	...
	getFicha:function(){
		...
		obj.getData(service,params,function(data){
            ...
            //var geoLevel=data.geographical.length;
            if (data.data.values.geographical.length==3) {
                obj.currentData.geo = {id:'00',label:'Nacional'};	
            }else{
                    obj.currentData.geo = {id:data.data.values.geographical[0].value,label:data.data.values.geographical[1].value};
                 }
            
            if (!obj.existGeo(vals.lastPoint.cvegeo)) {
                for (var x in data.data.values.geographical){
                    var label=data.data.values.geographical[x].label;
                    
                    if ((label=="Ageb")||(label=="Localidad")||(label=="Municipio")||(label=="Entidad")||(label=="Nombre")&&(cont==0)) {
                        if (cont==0) {
                            obj.currentData.edoList.push({cvegeo:vals.lastPoint.cvegeo,label:data.data.values.geographical[x].value});
                            cont=cont+1;
                        }
               ...
               /* if(data.geographical.length == 2){
					if(!obj.existGeo(vals.lastPoint.cvegeo))
						obj.currentData.edoList.push({cvegeo:vals.lastPoint.cvegeo,label:data.geographical[1].value});
                        obj.currentData.geo = {id:'00',label:'Nacional'};	
				}else{
					if(!obj.existGeo(vals.lastPoint.cvegeo))
                        //verificar si el corte es localidad
                        
                        if (data.geographical[1].label=="Localidad") {
                           obj.currentData.edoList.push({cvegeo:vals.lastPoint.cvegeo,label:data.geographical[1].value});
                        }else{
                            obj.currentData.edoList.push({cvegeo:vals.lastPoint.cvegeo,label:data.geographical[3].value});
                        }
					    obj.currentData.geo = {id:data.geographical[0].value,label:data.geographical[1].value};
				}*/
               
				obj.printFicha(data.data.values);	
				if(obj.cont<=obj.options.anio.length-2){
					obj.cont++; 
					obj.getFicha();
				}
		});
	...
	printFicha:function(data){
        ...
        if (data.economical==null) {
            data.economical=[];
            //data.economical.value={};
            var labels=["V01","V02","V03","v04","V05","V06","V07","V08","V09","V10","V11","V12","r01","r02","r03","r04","r05","r06","r07","r08","r09","r10","r11"];
            var legenda="<div class='legenda' style='position:absolute; bottom:5px'><div style='font-size:14px;font-weight:bold'>-- Información no disponible</div></div>";
            var contar=0;
            var sinDato=true;
           
            while (contar<23) {
               data.economical.push({label:labels[contar],value:"--"});
               contar=contar+1;
            }
            $('.infoCard_mainContainer').append(legenda);
        }
...
//			return symbol;
	...
	buildTabulado:function(data,id_source){
		...
		for (var x in data){
            if (data[x].label=="positionState") {
                $("#analysisCard_participacion").html(data[x].value);
                $("#PositionState").show();
            }
		...
		$("#"+id_source+'_data .tab-analiticas-container').width()+1;
	...
	creaGrafica:function(data, variableName){
		...
		for (var x in obj.options.anio){
			...
			if (currentYear!=obj.sinDatoYear ) {
                var tempData=[];
                for(var y in edosyMun){
                    var currentEdos= edosyMun[y];	
                    tempData.push(parseFloat(obj.databyYear[currentYear][currentEdos][variableName]));
                }	
		...
		$('.analysisCard_graphic').highcharts({
				...
				tooltip: {
                   ...
                   formatter: function () {
                        var punto=this.points;
                        for(var x in punto){
                            var yPoint=punto[x].point.y;
                            if (yPoint<0) {
                                punto[x].y='*';
                            }
                        }
                        
                        return '<b style="color:#2f7ed8">2004 </b><b>'+obj.formatNumber(punto[0].y)+'</b><br>'+
                               '<b style="color:#0d233a">2009 </b><b>'+obj.formatNumber(punto[1].y)+'</b><br>'+
                               '<b style="color:#8bbc21">2014 </b><b>'+obj.formatNumber(punto[2].y)+'</b><br>';
	...
	buildAnalysisCard:function() {
		...
		var chain='<div class="analysisCard_geographic">'+
				  ...
				  '<div class="analysisCard_datosComparativos">'+
						...
						'<div id="PositionState" class="analysisCard_title" style="display:none">Posici&oacute;n: <div id="analysisCard_participacion" class="analysisCard_datoComparativo"></div></div>'+
				  '</div>'+
	...
	exportaXLS:function(){
		...
		var headerTable=x;
			...
			if((headerTable!='Localidad')&&(headerTable!='Municipio')&&(headerTable!='Participacion')){
				if((headerTable!='Estado')&&(headerTable!='Clave')&&(headerTable!='Ageb')){
		...
		var currentCve=obj.currentData.edoList[0].cvegeo;
			for( var z in tempColumns){
			    if(tempColumns[z]=='Clave'){
				 var valor=	obj.currentData.edoList[y].cvegeo;
				}else{
					 var valor=obj.currentData.varData[tempColumns[z]][parseInt(y)][1];
					 }
				if((tempColumns[z]!='Municipio')&&(tempColumns[z]!='Participacion')&&(tempColumns[z]!='Clave')){
		...
		var yearPos=vals.yearPos;
		var titleYear= vals.years[yearPos];
		response.title=currentEdo+' Censos Económicos '+titleYear;
		response.values.push([""]);
		response.values.push(['fuente: Censos Económicos '+titleYear]);
			  ...
			  success:function(json,estatus){
				...
				if(json){
						...
						//if(contador<=years.length-1){
                            ...
                            for (var x in values){
                                if (values[x].label=="elements") {
                                    //code
                                    if (values[x].value.length>0){
										//var values=json.data.values;
                                        obj.saveYears(values,id,years[contador]);
                                        
                                    }else{obj.sinDatoYear=years[contador];}
                                }
                            }
