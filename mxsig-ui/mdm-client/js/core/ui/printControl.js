	...
	return{
		...
		printMap:function(){
			...
			if (mapPointer){
				...
				//MDM6('updateSize');
				var evento = function(img){
					cadena = '<img src="'+img+'" class="mapPrint">';
					...
					var width = $("#map").width();
					var times = (height/556.8);
					var widthImg = width/times;
					var left = '0px';
					var sizeWidth = (/firefox/.test(navigator.userAgent.toLowerCase()))?720:960;
					if (widthImg>sizeWidth) {
                        left  = ('-'+(widthImg-sizeWidth)/2)+'px';
                    }
					$(".mapPrint").css('left',left);
					obj.printEvent();
				}
				MDM6('exportMap','png',{event:evento,message:'Imprimiendo mapa...'});
				//$('#map').removeClass('mapPrint');
				//MDM6('updateSize');
			}
		},
		//-------------------------------------------------------------------
		...
		injectStructureBlank:function(content){
			var obj = this;
			$('#printContainer').remove();
			$('body').append(
		...
		getViewDataAsDom:function(){
			...
			var cadena = '<div id="print_page_break" class="print-page-break"></div>';
				cadena+= '<div class="print-title">'+sources.proyAlias+'</div>';
