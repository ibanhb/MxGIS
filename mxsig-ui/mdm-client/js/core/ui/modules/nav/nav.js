define(function(){
	...
	return {
		...
		goToUrlParams:function(){
			  ...
			  if(lon && lat && z){
				  ...
				  setTimeout(function(){
					...
					var zoom = parseInt(z,10);
					if ((width>=1500)&&(zoom==1)) {
						zoom+=1;
					}
				  	obj.map.goCoords(merc.lon,merc.lat,{zoomLevel:zoom});
				  },1000);
			  }
		},
		...
		mapMove:function(mode){
			var obj = this;
			var okControl = (obj.config.system && obj.config.system.urlShareVars);
			
			if(saveHistory && isReady && okControl ){
